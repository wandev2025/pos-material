...
export default function PembelianScreen() {
  ...
  useEffect(() => { if (isManager) loadData(); }, [isManager]);
  ...
  const statusColor = (s: string) =>
    s === 'PAID' ? { bg: '#F0FDF4', fg: '#166534' }
    : s === 'PARTIAL' ? { bg: '#FFFBEB', fg: '#B45309' }
    : { bg: '#FEF2F2', fg: '#DC2626' };
  ...
  const renderForm = () => (
    <View style={styles.card}>
      ...
      <Text style={styles.cardHint}>Catat barang yang diterima dari supplier. Stok naik & hutang supplier tercatat otomatis.</Text>
          ...
          <View key={row._id} style={[styles.mCard, { zIndex: activeRowId === row._id ? 100 : 1 }]}>
            ...
            <View style={styles.mRow}>
              <View style={{ flex: 1, marginRight: 8 }}>
                ...
                <TextInput style={styles.input} keyboardType="numeric" value={row.qty} onChangeText={t => updateRow(row._id, 'qty', t)} />
              </View>
              <View style={{ flex: 1 }}>
                <Text style={styles.mLabel}>HARGA BELI</Text>
                <TextInput style={styles.input} keyboardType="numeric" value={row.cost} onChangeText={t => updateRow(row._id, 'cost', t)} placeholder="0" />
              </View>
            </View>
      ...
      <View style={[styles.row, { marginTop: 14 }]}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <TextInput style={styles.input} keyboardType="numeric" value={paidStr} onChangeText={setPaidStr} placeholder="0" />
        </View>
      </View>
      ...
      <View style={styles.summaryBox}>
        <View style={styles.summaryRow}><Text style={styles.summaryLabel}>Total Pembelian</Text><Text style={styles.summaryVal}>{formatRupiah(total)}</Text></View>
        <View style={styles.summaryRow}><Text style={styles.summaryLabel}>Dibayar</Text><Text style={styles.summaryVal}>{formatRupiah(paid)}</Text></View>
        <View style={[styles.summaryRow, { marginTop: 4 }]}>
          <Text style={[styles.summaryLabel, { fontWeight: '900', color: '#0F172A' }]}>Sisa Hutang</Text>
          <Text style={[styles.summaryVal, { color: outstanding > 0 ? '#DC2626' : '#16A34A', fontSize: 18 }]}>{formatRupiah(outstanding)}</Text>
        </View>
        <View style={[styles.statusPill, { backgroundColor: statusColor(status).bg, alignSelf: 'flex-start', marginTop: 8 }]}>
      ...
      {!online && <Text style={styles.offlineHint}>Tidak ada koneksi internet — penyimpanan dinonaktifkan sementara.</Text>}
    </View>
  );
  ...
  const renderRecent = () => (
    <View style={styles.card}>
      <View style={styles.rowBetween}>
        ...
        <TouchableOpacity onPress={loadData}><Feather name="refresh-cw" size={16} color="#64748B" /></TouchableOpacity>
      </View>
      {loading ? <ActivityIndicator color="#DC2626" style={{ marginTop: 20 }} /> : (
        purchases.length === 0 ? <Text style={styles.empty}>Belum ada pembelian.</Text> :
          purchases.map(p => {
            ...
            return (
              <View key={p.id} style={styles.purchaseCard}>
                <View style={styles.rowBetween}>
                  <View style={{ flex: 1 }}>
                    <Text style={styles.purchaseSupplier} numberOfLines={1}>{p.supplier_name || 'Supplier'}</Text>
                    <Text style={styles.purchaseMeta}>
                      {!!p.invoice_no && `#${p.invoice_no} • `}{new Date(p.created_at).toLocaleDateString('id-ID')}
                    </Text>
                  </View>
                ...
                <View style={styles.purchaseFooter}>
                  <View><Text style={styles.pfLabel}>Total</Text><Text style={styles.pfVal}>{formatRupiah(p.total_amount)}</Text></View>
                  <View><Text style={styles.pfLabel}>Dibayar</Text><Text style={styles.pfVal}>{formatRupiah(p.paid_amount)}</Text></View>
                  <View><Text style={styles.pfLabel}>Sisa Hutang</Text><Text style={[styles.pfVal, { color: owed > 0 ? '#DC2626' : '#16A34A' }]}>{formatRupiah(owed)}</Text></View>
                </View>
              </View>
            );
          })
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: '#FFF', borderRadius: 20, padding: 20, marginBottom: 16, borderWidth: 1, borderColor: '#E5E7EB' },
  ...
  input: { backgroundColor: '#F9FAFB', borderWidth: 1, borderColor: '#E5E7EB', borderRadius: 12, padding: 12, fontSize: 15, color: '#111827', marginBottom: 12 },
  inputIconWrap: { flexDirection: 'row', alignItems: 'center', gap: 10, backgroundColor: '#F9FAFB', borderWidth: 1, borderColor: '#E5E7EB', borderRadius: 12, paddingHorizontal: 12, marginBottom: 12 },
  inputFlex: { flex: 1, paddingVertical: 12, fontSize: 15, color: '#111827', outlineStyle: 'none' } as any,
  row: { flexDirection: 'row' },
  rowBetween: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },

  // Item table (desktop)
  tableHead: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 4, paddingBottom: 8, borderBottomWidth: 1, borderBottomColor: '#F1F5F9' },
  ...
  cellInput: { backgroundColor: '#F9FAFB', borderWidth: 1, borderColor: '#E5E7EB', borderRadius: 10, paddingHorizontal: 10, paddingVertical: 10, fontSize: 14, color: '#111827' },
  ...
  // Item card (mobile)
  mCard: { backgroundColor: '#F9FAFB', borderWidth: 1, borderColor: '#E5E7EB', borderRadius: 14, padding: 12, marginBottom: 10 },
  ...
  // Dropdown
  dropdown: { position: 'absolute', top: 46, left: 0, right: 0, backgroundColor: '#FFF', borderRadius: 10, borderWidth: 1, borderColor: '#E2E8F0', zIndex: 1000, elevation: 10 },
  ...
  purchaseCard: { backgroundColor: '#F9FAFB', borderWidth: 1, borderColor: '#E5E7EB', borderRadius: 14, padding: 14, marginTop: 12 },
