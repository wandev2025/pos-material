...
const emptyRow = (): PurchaseRow => ({ _id: generateId(), item: null, query: '', qty: '1', cost: '', total: '0' });

export default function PembelianScreen() {
  const { profile } = useProfile();
  const { width } = useWindowDimensions();
  const isDesktop = width > 900;
  const isManager = atLeast(profile?.role, 'ADMIN'); 
  ...
  // --- SUPPLIER MODAL & MANAGEMENT STATE ---
  ...
  const openEditSupplier = (s: Supplier) => {
    ...
    setNsName(s.name);
    setNsPhone(s.phone || '');
    setNsAddress(s.address || '');
    setShowAddSupplier(true);
  };

  const handleSaveSupplier = async () => {
    if (!nsName.trim()) return toast.error('Nama supplier wajib diisi.');
    setSaving(true);
    try {
      if (editingSupplier) {
        ...
        if (error) throw error;
        if (data) selectSupplier(data as Supplier);
  ...
  const handleDeleteSupplier = async (id: number) => {
    const ok = await confirm({
      ...
      message: 'Anda yakin ingin menghapus supplier ini? Tindakan ini tidak dapat dibatalkan.',
      danger: true,
    });
    if (!ok) return;
    ...
    try {
      ...
      toast.error('Supplier tidak bisa dihapus karena masih terkait dengan data pembelian.');
    }
  };

  const handleCompletePayment = async () => {
    if (!payingPurchase) return;
    const additional = parseNum(payAmountStr);
    if (additional <= 0) return toast.error('Masukkan jumlah pembayaran.');

    const newPaid = (payingPurchase.paid_amount || 0) + additional;
    const newStatus = computeStatus(payingPurchase.total_amount, newPaid);
  ...
  const selectInvItem = (item: InventoryItem, rowId: string) => {
    ...
    setRows(prev => {
      const next = prev.map(r => {
        ...
        // Modified: cost is set to empty string instead of item.cost
        const cost = ''; 
  ...
  // Inline-create an inventory item
  ...
  const handleCreateItem = async () => {
    ...
    setNewItemFor(null);
    selectInvItem(data as InventoryItem, rowId);
  };
  ...
  const renderForm = () => (
    <View style={styles.card}>
      ...
      <View style={styles.rowBetween}>
        ...
        <TouchableOpacity 
          onPress={() => setShowManageSuppliers(true)}
          style={styles.manageBtn}
        >
          <Feather name="settings" size={12} color="#DC2626" />
      ...
      <CommandPalette<Supplier>
        ...
        createLabel={t => `Tambah detail supplier "${t}"`}
      />
      ...
      {rows.map(row =>
        isDesktop ? (
          <View key={row._id} style={styles.tableRow}>
            <TouchableOpacity
              style={[styles.cellInput, { flex: 2.6, justifyContent: 'center', marginRight: 8, minWidth: 0 }]}
            ...
            <TextInput
              style={[styles.cellInput, { flex: 0.9, textAlign: 'center', marginRight: 8, minWidth: 0 }]}
              keyboardType="numeric"
              value={row.qty}
              onChangeText={t => updateRow(row._id, 'qty', t)}
            />
            <TextInput
              style={[styles.cellInput, { flex: 1.3, textAlign: 'center', marginRight: 8, minWidth: 0 }]}
              keyboardType="numeric"
              value={row.cost}
              onChangeText={t => updateRow(row._id, 'cost', t)}
              placeholder="0"
            />
            <View style={{ flex: 1.3, minWidth: 0, alignItems: 'flex-end' }}>
                <Text style={styles.cellTotal} numberOfLines={1}>{formatRupiah(parseNum(row.total))}</Text>
            </View>
            ...
            <View key={p.id} style={styles.purchaseCard}>
              ...
              {owed > 0 && (
                <TouchableOpacity 
                  ...
                  onPress={() => {
                    setPayingPurchase(p);
                    setPayAmountStr(owed.toString());
                  }}
                >
                  <Feather name="credit-card" size={12} color="#DC2626" />
                  <Text style={styles.payNowBtnText}>BAYAR SISA</Text>
                </TouchableOpacity>
              )}
            </View>
          );
  ...
  return (
    <>
      ...
      <Modal visible={showManageSuppliers} animationType="slide" transparent onRequestClose={() => setShowManageSuppliers(false)}>
        <View style={styles.modalOverlay}>
          <View style={[styles.modalCard, { height: '80%', maxWidth: 600 }]}>
            <View style={styles.modalHead}>
              <Text style={styles.modalTitle}>Daftar Supplier</Text>
              <TouchableOpacity onPress={() => setShowManageSuppliers(false)}>
                <Feather name="x" size={24} color="#0F172A" />
              </TouchableOpacity>
            </View>
            
            <View style={styles.supSearchBox}>
              <Feather name="search" size={16} color="#94A3B8" />
              <TextInput 
                style={styles.supSearchInput} 
            ...
            <ScrollView style={{ flex: 1 }}>
              {filteredSuppliers.map(s => (
                <View key={s.id} style={styles.supListItem}>
                  <View style={{ flex: 1 }}>
                    <Text style={styles.supListName}>{s.name}</Text>
                    {s.phone && <Text style={styles.supListSub}>{s.phone}</Text>}
                  </View>
                  <View style={styles.supListActions}>
                    <TouchableOpacity onPress={() => openEditSupplier(s)} style={styles.iconAction}>
                      <Feather name="edit-2" size={16} color="#475569" />
                    </TouchableOpacity>
                    <TouchableOpacity onPress={() => handleDeleteSupplier(s.id)} style={styles.iconAction}>
              ...
              {filteredSuppliers.length === 0 && (
                <Text style={styles.empty}>Supplier tidak ditemukan.</Text>
              )}
            </ScrollView>
            ...
            >
              <Text style={styles.btnText}>+ TAMBAH SUPPLIER BARU</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
      ...
      {/* MODAL: PAY REMAINING */}
      <Modal visible={!!payingPurchase} transparent animationType="fade" onRequestClose={() => setPayingPurchase(null)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            <View style={styles.modalHead}>
              <Text style={styles.modalTitle}>Bayar Sisa Hutang</Text>
              <TouchableOpacity onPress={() => setPayingPurchase(null)}>
                <Feather name="x" size={22} color="#0F172A" />
              </TouchableOpacity>
            </View>
            <Text style={styles.label}>Supplier: {payingPurchase?.supplier_name}</Text>
            <Text style={styles.label}>Total Sisa: {formatRupiah((payingPurchase?.total_amount || 0) - (payingPurchase?.paid_amount || 0))}</Text>
            ...
            <TouchableOpacity
              ...
              disabled={saving}
            >
              {saving ? <ActivityIndicator color="#FFF" /> : <Text style={styles.btnText}>PROSES PEMBAYARAN</Text>}
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
      ...
      <Modal visible={showAddSupplier} transparent animationType="fade" onRequestClose={() => setShowAddSupplier(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            <View style={styles.modalHead}>
              <Text style={styles.modalTitle}>{editingSupplier ? 'Edit Supplier' : 'Supplier Baru'}</Text>
              <TouchableOpacity onPress={() => setShowAddSupplier(false)}>
                <Feather name="x" size={22} color="#0F172A" />
              </TouchableOpacity>
            </View>
            <Text style={styles.label}>Nama Supplier</Text>
            <TextInput style={styles.input} value={nsName} onChangeText={setNsName} placeholder="PT. Nama Supplier" />
            <Text style={styles.label}>No. Telepon</Text>
            <TextInput style={styles.input} value={nsPhone} onChangeText={setNsPhone} keyboardType="phone-pad" placeholder="08..." />
            ...
            <TouchableOpacity
              ...
              disabled={saving}
            >
              {saving ? <ActivityIndicator color="#FFF" /> : <Text style={styles.btnText}>SIMPAN DATA</Text>}
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
...
const styles = StyleSheet.create({
  ...
  // Recent purchases
  empty: { color: '#94A3B8', fontStyle: 'italic', paddingVertical: 12, textAlign: 'center' },
  purchaseCard: {
    backgroundColor: '#F9FAFB',
    borderWidth: 1,
    borderColor: '#E5E7EB',
  ...
  // Manage Supplier Styles
  manageBtn: { flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: '#FEF2F2', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8 },
  manageBtnText: { fontSize: 11, fontWeight: '800', color: '#DC2626' },
  supSearchBox: { flexDirection: 'row', alignItems: 'center', gap: 10, backgroundColor: '#F9FAFB', borderRadius: 12, paddingHorizontal: 12, height: 44, marginBottom: 16, borderWidth: 1, borderColor: '#E5E7EB' },
  supSearchInput: { flex: 1, fontSize: 15, color: '#0F172A' },
  supListItem: { flexDirection: 'row', alignItems: 'center', paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: '#F1F5F9' },
  supListName: { fontSize: 15, fontWeight: '700', color: '#0F172A' },
  supListSub: { fontSize: 12, color: '#64748B', marginTop: 2 },
  supListActions: { flexDirection: 'row', gap: 12 },
  ...
  payNowBtnText: { fontSize: 11, fontWeight: '900', color: '#DC2626' },
});