    ...
    return (
      <View style={styles.webContainer}>
        ...
        <View style={[styles.sidebar, { width: collapsed ? 85 : 260 }]}>
          ...
          <TouchableOpacity style={styles.toggleBtn} onPress={() => setCollapsed(!collapsed)} activeOpacity={0.8}>
      ...
      >
        ...
        <Tabs.Screen
          ...
          options={{
            href: isManager ? undefined : null,
            title: 'Retur',
            tabBarIcon: ({ color }) => <Feather name="corner-up-left" size={20} color={color} />,
          }}
        />
      </Tabs>
      <FloatingTabBar />
...
function FloatingTabBar() {
  ...
  const renderRow = (circle: 'grid' | 'close', wrapStyle: any, pe: 'box-none' | 'auto') => (
    <View style={[wrapStyle, { paddingBottom: Math.max(insets.bottom, 10) }]} pointerEvents={pe}>
      ...
      >
        <Feather
          name={circle === 'grid' ? 'grid' : 'x'}
          size={22}
          color={circle === 'close' || onSecondary ? '#FFF' : '#0F172A'}
        />
      </PressableScale>
    </View>
  );
                ...
                return (
                  ...
                  >
                    <PressableScale onPress={() => go(item.route)} style={{ alignItems: 'center', gap: 8 }}>
                      ...
                      <Text style={tabStyles.tileLabel} numberOfLines={1}>
                        {item.label}
                      </Text>
                    </PressableScale>
                  </Animated.View>
