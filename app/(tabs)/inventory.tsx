...
export default function InventoryScreen() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [invRes, metRes, catRes] = await Promise.all([
        supabase.from('inventory').select('*, metrics(unit_name)').order('item_name'),
      ...
      if (metRes.data) {
        ...
        if (metRes.data.length > 0 && formMetricId === '') setFormMetricId(metRes.data[0].id.toString());
      }
  ...
  useEffect(() => {
    setVisibleCount(20);
  }, [search, invFilter, invSort, selectedCatFilter]);
  ...
  const filtered = useMemo(() => {
    ...
    if (selectedCatFilter) {
        list = list.filter(i => i.category === selectedCatFilter);
    }
  ...
  }, [inventory, search, invFilter, invSort, selectedCatFilter]);
    ...
    <View style={styles.container}>
      ...
      <View style={{ backgroundColor: '#FFF', borderBottomWidth: 1, borderBottomColor: '#E5E7EB' }}>
        <ScrollView 
            ...
            contentContainerStyle={{ paddingHorizontal: isDesktop ? 40 : 14, paddingBottom: 12, gap: 8 }}
        >
            <TouchableOpacity
                onPress={() => setSelectedCatFilter(null)}
                style={[styles.fChip, !selectedCatFilter && styles.fChipActive]}
            >
                <Text style={[styles.fChipText, !selectedCatFilter && styles.fChipTextActive]}>Semua</Text>
            </TouchableOpacity>
            
            {dbCategories.map(cat => (
                <TouchableOpacity
                    key={cat.id}
                    onPress={() => setSelectedCatFilter(cat.name)}
                    style={[styles.fChip, selectedCatFilter === cat.name && styles.fChipActive]}
                >
                    <Text style={[styles.fChipText, selectedCatFilter === cat.name && styles.fChipTextActive]}>
                        {cat.name}
                    </Text>
                </TouchableOpacity>
            ))}
              ...
              <View style={styles.unitWrap}>
                {dbCategories.map(cat => (
                  <TouchableOpacity
                    ...
                    style={[styles.unitChip, formCategory === cat.name && styles.unitChipActive]}
                  >
                    <Text style={[styles.unitChipText, formCategory === cat.name && styles.unitChipTextActive]}>
                        {cat.name}
                    </Text>
                  </TouchableOpacity>
                ))}
