  ...
  const load = async () => {
    ...
    if (ids.length) {
      const itemsRes = await supabase
        .from('sale_items')
        .select('item_name, quantity, price_at_sale')
        .in('sale_id', ids);
        ...
        <Animated.View entering={FadeIn.duration(260)}>
          ...
          <View style={styles.summaryRow}>
            <View style={styles.summaryCard}>
              ...
              <Text style={styles.summaryVal} numberOfLines={1} adjustsFontSizeToFit>
                {summary.count}
              </Text>
            </View>
            <View style={styles.summaryCard}>
          ...
          <View style={styles.card}>
            ...
            {topItems.length === 0 ? (
              <Text style={styles.empty}>Belum ada penjualan.</Text>
            ) : (
              topItems.map((t, i) => (
          ...
          <View style={styles.card}>
            ...
            {byPayment.map(p => (
              <View key={p.label} style={styles.listRow}>
                <View style={{ flex: 1 }}>
                  <Text style={styles.listName}>{p.label}</Text>
                  <Text style={styles.listSub}>{p.count} transaksi</Text>
                </View>
            ...
            {byCashier.map(c => (
              <View key={c.label} style={styles.listRow}>
                <View style={{ flex: 1 }}>
                  <Text style={styles.listName}>{c.label}</Text>
                  <Text style={styles.listSub}>{c.count} transaksi</Text>
                </View>
          ...
          <View style={[styles.card, { borderColor: '#FED7AA', borderWidth: 1 }]}>
            ...
            {piutang.rows.length === 0 ? (
              <Text style={styles.empty}>Tidak ada piutang. 🎉</Text>
            ) : (
              piutang.rows.map(r => (
