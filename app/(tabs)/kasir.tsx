...
export default function KasirScreen() {
  ...
  const expectedLive = useMemo(() => (session ? (session.opening_float || 0) + cashSales : 0), [session, cashSales]);
  const varianceLive = useMemo(() => parseNum(countedCash) - expectedLive, [countedCash, expectedLive]);
  ...
  if (!me) {
    return (
      <View style={styles.center}>
        <Text style={styles.denied}>Profil kasir tidak ditemukan.</Text>
      </View>
    );
  }
          ...
          {closeResult && (
            <View style={styles.resultCard}>
              ...
              <View style={[styles.resLine, styles.resLineTop]}>
                ...
                >
                  {(closeResult.variance || 0) > 0 ? '+' : ''}
                  {formatRupiah(closeResult.variance || 0)}
                </Text>
              </View>
            </View>
          )}
          ...
          {!session ? (
            <View style={styles.card}>
              ...
              <Text style={styles.preview}>
                Modal: <Text style={styles.previewStrong}>{formatRupiah(parseNum(openingFloat))}</Text>
              </Text>
              ...
              {!online && (
                <Text style={styles.offlineHint}>Tidak ada koneksi internet — buka kasir dinonaktifkan.</Text>
              )}
            </View>
          ) : (
            <>
              ...
              <View style={styles.card}>
                ...
                <Text style={styles.note}>
                  Perkiraan ini dari penjualan tunai shift ini. Total final (termasuk pembayaran piutang & retur)
                  dihitung saat kasir ditutup.
                </Text>
              </View>
              ...
              <View style={styles.card}>
                ...
                <Text style={styles.hint}>
                  Hitung uang fisik di laci, lalu masukkan jumlahnya untuk menghitung selisih.
                </Text>
                ...
                {countedCash !== '' && (
                  <View style={styles.varianceBox}>
                    ...
                    <Text
                      style={[
                        styles.varianceVal,
                        { color: varianceLive === 0 ? '#059669' : varianceLive > 0 ? '#2563EB' : '#DC2626' },
                      ]}
                    >
                      {varianceLive > 0 ? '+' : ''}
                      {formatRupiah(varianceLive)}
                    </Text>
                  </View>
                )}
                ...
                >
                  {saving ? (
                    ...
                    <Text style={styles.btnText}>TUTUP KASIR SEKARANG</Text>
                  )}
                </TouchableOpacity>
                {!online && (
                  <Text style={styles.offlineHint}>Tidak ada koneksi internet — tutup kasir dinonaktifkan.</Text>
                )}
              </View>
            </>
          )}
                  ...
                  <View key={s.id} style={styles.listRow}>
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={styles.listSub}>
                        Diharapkan {formatRupiah(s.expected_cash || 0)} • Dihitung {formatRupiah(s.counted_cash || 0)}
                      </Text>
                    </View>
                    <Text style={[styles.listVal, { color: vColor }]}>
                      {v > 0 ? '+' : ''}
                      {formatRupiah(v)}
                    </Text>
                  </View>
