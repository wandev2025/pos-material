import {
  DEFAULT_PRINT_CONFIG, listAgentPrinters, pingAgent,
  pairWebSerial, pairWebUsb, printDocument
} from '@/lib/printing';
...
export default function SetupScreen() {
  ...
  const [pairedDevices, setPairedDevices] = useState<{ WEBUSB: string | null; WEBSERIAL: string | null }>({ WEBUSB: null, WEBSERIAL: null });
  ...
  const refreshPaired = async () => {
    const [usb, serial] = await Promise.all([
      getPairedDevice('WEBUSB'),
      getPairedDevice('WEBSERIAL'),
    ]);
  ...
  const renderDocConfig = (doc: DocType, title: string, allowPaper: boolean) => {
    ...
    return (
      <View style={styles.docCard} key={doc}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ flexDirection: 'row', marginTop: 5, marginBottom: 14 }}>
          {METHOD_OPTIONS.map(m => (
            <TouchableOpacity key={m} onPress={() => updateDocConfig(doc, { transport: m })} style={[styles.chip, cfg.transport === m && styles.chipActive]}>
        ...
        {isPaired && (
          <View style={{ marginBottom: 16 }}>
            ...
            <Text style={styles.pairedText}>{pairedDevices[cfg.transport as 'WEBUSB' | 'WEBSERIAL'] || 'Belum ada perangkat'}</Text>
            <View style={styles.row}>
              <TouchableOpacity style={styles.pairBtn} onPress={() => handlePair(cfg.transport as 'WEBUSB' | 'WEBSERIAL')}>
                <Feather name="link" size={14} color="#FFF" />
                <Text style={styles.pairBtnText}>Pasang Printer</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.forgetBtn} onPress={() => handleForget(cfg.transport as 'WEBUSB' | 'WEBSERIAL')}>
          ...
          <View style={{ marginBottom: 16 }}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ flexDirection: 'row', marginTop: 5 }}>
              {PAPER_OPTIONS.map(p => {
                ...
                return (
                  <TouchableOpacity key={p.id} onPress={() => updateDocConfig(doc, { paper: p.id })} style={[styles.chip, active && styles.chipActive]}>
      ...
      <ScrollView style={[styles.content, !isDesktop && styles.contentMobile]}>
        ...
        {activeTab === 'PRINTERS' && (
          <View style={[styles.card, !isDesktop && styles.cardMobile]}>
            ...
            {usesAgent && (bridgeStatus === 'OFFLINE' ? (
              <View style={styles.statusBoxError}>
                ...
                <Text style={styles.noteText}>Disarankan menjalankan agent dari folder agent/ (lebih stabil daripada .bat ini).</Text>
              </View>
            ) : (
...
const styles = StyleSheet.create({
  ...
  docCard: { backgroundColor: '#F8FAFC', borderWidth: 1, borderColor: '#E2E8F0', borderRadius: 14, padding: 18, marginBottom: 16 },
  ...
  pairBtn: { backgroundColor: '#1E293B', flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, paddingVertical: 10, borderRadius: 8, gap: 8 },
  pairBtnText: { color: '#FFF', fontWeight: '700', fontSize: 12 },
  forgetBtn: { backgroundColor: '#FEF2F2', flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, paddingVertical: 10, borderRadius: 8, gap: 8, marginLeft: 10, borderWidth: 1, borderColor: '#FECACA' },
  forgetBtnText: { color: '#DC2626', fontWeight: '700', fontSize: 12 },
  testBtn: { backgroundColor: '#E2E8F0', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', padding: 12, borderRadius: 8, gap: 8 },
