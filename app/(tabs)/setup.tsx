...
// --- TYPES ---
type TabType = 'INFO' | 'PRINTERS' | 'METRICS' | 'PAYMENT' | 'CATEGORIES';
...
export default function SetupScreen() {
  ...
  const upsertCategory = async () => {
    ...
    if (editId) {
      await supabase.from('categories').update({ name: newCat }).eq('id', editId);
  ...
  return (
    <View style={[styles.container, isDesktop && styles.row]}>
      ...
      <View style={[styles.subSidebar, isDesktop ? { width: 260 } : { width: '100%' }]}>
        ...
        <SubBtn id="CATEGORIES" label="Kategori Barang" icon="tag" active={activeTab} set={setActiveTab} />
      </View>
      ...
      >
        ...
        {/* TAB: METRICS, PAYMENT, CATEGORIES */}
        {(activeTab === 'METRICS' || activeTab === 'PAYMENT' || activeTab === 'CATEGORIES') && (
          <View style={[styles.card, !isDesktop && styles.cardMobile]}>
            <Text style={styles.cardTitle}>
                {activeTab === 'METRICS' ? 'Manajemen Satuan' : 
            ...
            <View style={styles.row}>
              <TextInput
                ...
                placeholder={
                    activeTab === 'METRICS' ? 'Contoh: Pcs, Box, Kg' : 
                    activeTab === 'PAYMENT' ? 'Contoh: Tunai, BCA' : 
                    'Nama Kategori...'
                }
                value={
                    activeTab === 'METRICS' ? newUnit : 
                    activeTab === 'PAYMENT' ? newPayment : 
                    newCat
                }
                onChangeText={
                    activeTab === 'METRICS' ? setNewUnit : 
                    activeTab === 'PAYMENT' ? setNewPayment : 
                    setNewCat
                }
              />
              <TouchableOpacity style={styles.darkBtn} onPress={
                  activeTab === 'METRICS' ? addMetric : 
                  activeTab === 'PAYMENT' ? addPayment : 
                  upsertCategory
              }>
                <Text style={styles.btnText}>{editId ? 'Simpan' : 'Tambah'}</Text>
              </TouchableOpacity>
              {editId && (
                <TouchableOpacity style={[styles.darkBtn, { backgroundColor: '#64748B' }]} onPress={() => {setEditId(null); setNewCat('');}}>
                  <Text style={styles.btnText}>Batal</Text>
                </TouchableOpacity>
              )}
            </View>
            <View style={{ marginTop: 20 }}>
              {(activeTab === 'METRICS' ? metrics : activeTab === 'PAYMENT' ? payments : categories).map((item: any) => (
                <View key={item.id} style={styles.listRow}>
                  <Text style={{ fontWeight: '700', color: '#334155' }}>
                    {(activeTab === 'METRICS' ? item.unit_name : activeTab === 'PAYMENT' ? item.name : item.name).toUpperCase()}
                  </Text>
                  <View style={{ flexDirection: 'row', gap: 15 }}>
                    {activeTab === 'CATEGORIES' && (
                        <TouchableOpacity onPress={() => {setEditId(item.id); setNewCat(item.name);}}>
                    ...
                    <TouchableOpacity
                        onPress={() => deleteItem(
                            ...
                            activeTab === 'PAYMENT' ? 'payment_methods' : 
                            'categories', 
                            item.id
                        )}
                    >
