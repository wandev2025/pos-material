...
export default function PelangganScreen() {
  ...
  useEffect(() => {
    if (isManager) load(); /* eslint-disable-next-line */
  }, [isManager]);
  ...
  type Ledger = {
    notaRows: { sale: CreditSale; paidForSale: number; outstanding: number }[];
    pays: CustomerPayment[];
    net: number;
  };
  const ledgers = useMemo(() => {
    ...
    payments.forEach(p => {
      ...
      if (list) list.push(p);
      else paysByCustomer.set(p.customer_id, [p]);
    ...
    creditSales.forEach(s => {
      ...
      if (list) list.push(s);
      else notasByCustomer.set(s.customer_id, [s]);
    });
    const map = new Map<number, Ledger>();
    customers.forEach(c => {
      const notas = (notasByCustomer.get(c.id) ?? [])
        .slice()
  ...
  const totalPiutang = useMemo(() => Array.from(outstandingMap.values()).reduce((a, b) => a + b, 0), [outstandingMap]);
    ...
    return (
      <ScrollView contentContainerStyle={{ paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
          ...
          <View style={{ flex: 1 }}>
            ...
            {!!selectedCustomer.phone && (
              <Text style={styles.ledgerMeta}>
                <Feather name="phone" size={11} color="#94A3B8" /> {selectedCustomer.phone}
              </Text>
            )}
            {!!selectedCustomer.address && (
              <Text style={styles.ledgerMeta}>
                <Feather name="map-pin" size={11} color="#94A3B8" /> {selectedCustomer.address}
              </Text>
            )}
          </View>
        ...
        <View style={styles.netCard}>
          ...
          <Text style={styles.netVal} numberOfLines={1} adjustsFontSizeToFit>
            {formatRupiah(net)}
          </Text>
        </View>
            ...
            <View key={n.sale.id} style={styles.notaCard}>
              <View style={{ flex: 1 }}>
                <View style={styles.notaTopRow}>
                  ...
                  <Text style={[styles.badge, n.sale.status === 'PARTIAL' ? styles.badgePartial : styles.badgeUnpaid]}>
                    {n.sale.status}
                  </Text>
                </View>
                <Text style={styles.notaDate}>
                  {new Date(n.sale.created_at).toLocaleString('id-ID')}
                  {n.sale.payment_method ? ` • ${n.sale.payment_method}` : ''}
                </Text>
                <Text style={styles.notaSub}>
                  Total {formatRupiah(n.sale.total_amount)} • Dibayar{' '}
                  {formatRupiah((n.sale.down_payment || 0) + n.paidForSale)}
                </Text>
              </View>
    ...
    return (
      <TouchableOpacity style={[styles.custCard, active && styles.custCardActive]} onPress={() => openCustomer(item)}>
        ...
        <View style={{ flex: 1 }}>
          ...
          {!!item.phone && (
            <Text style={styles.custPhone} numberOfLines={1}>
              {item.phone}
            </Text>
          )}
        </View>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              >
                {saving ? (
                  ...
                  <Text style={styles.primaryBtnText}>{editingCustomer ? 'SIMPAN PERUBAHAN' : 'SIMPAN PELANGGAN'}</Text>
                )}
              </TouchableOpacity>
              {!online && (
                <Text style={styles.offlineHint}>Tidak ada koneksi — penyimpanan dinonaktifkan sementara.</Text>
              )}
            </ScrollView>
          </KeyboardAvoidingView>
        </View>
          ...
          <KeyboardAvoidingView behavior="padding" style={[styles.modalContent, isDesktop && { width: 520 }]}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 16 }}>
                <TouchableOpacity
                  onPress={() => setPayNotaId(null)}
                  style={[styles.chip, payNotaId === null && styles.chipActive]}
                >
                ...
                {selectedLedger?.notaRows.map(n => {
                  ...
                  return (
                    <TouchableOpacity
                      key={n.sale.id}
                      onPress={() => {
                        setPayNotaId(n.sale.id);
                        setPayAmountStr(String(Math.max(0, Math.round(n.outstanding))));
                      }}
                      style={[styles.chip, active && styles.chipActive]}
                    >
                      <Text style={active ? styles.chipTextActive : styles.chipText}>
                        #{n.sale.id} • {formatRupiah(n.outstanding)}
                      </Text>
                    </TouchableOpacity>
                  );
                })}
              ...
              >
                {saving ? (
                  ...
                  <Text style={styles.primaryBtnText}>SIMPAN PEMBAYARAN</Text>
                )}
              </TouchableOpacity>
              {!online && (
                <Text style={styles.offlineHint}>Tidak ada koneksi — pencatatan dinonaktifkan sementara.</Text>
              )}
            </ScrollView>
          </KeyboardAvoidingView>
        </View>
