...
export default function UnifiedPOSHub() {
  ...
  const [discountRows, setDiscountRows] = useState<string[]>([]);
  const [showDiscount, setShowDiscount] = useState(false);
  const [rows, setRows] = useState<SaleRow[]>([{ _id: generateId(), item: null, query: '', qty: '1', price: '0', discount: '0', total: '0' }]);
  const [popularItems, setPopularItems] = useState<InventoryItem[]>([]);
  
  // Row Suggestions
  const [activeRowId, setActiveRowId] = useState<string | null>(null);
  const [inlineSuggestions, setInlineSuggestions] = useState<InventoryItem[]>([]);
  ...
  // Surat Jalan (DO) Selection State
  const [doModal, setDoModal] = useState(false);
  const [doSelectedItems, setDoSelectedItems] = useState<Record<number, number>>({});
  ...
  useEffect(() => {
    loadInitialData();
    loadPopular();
    loadPaletteCatalog(); // Preload catalog for inline search
  }, []);
  ...
  const isElectronic = !isTempo && !isCash;
    ...
    setRows(prev => {
      ...
      if (exists) {
        next = prev.map(r => {
          ...
          return { ...r, qty: String(qty), total: String(total), query: item.item_name };
        });
      } else {
        const newItem: SaleRow = {
          _id: generateId(),
          item,
        ...
        const emptyIdx = prev.findIndex(r => r.item === null);
        if (emptyIdx !== -1) {
          const copy = [...prev];
          copy.splice(emptyIdx, 0, newItem);
          next = copy;
        } else {
          next = [...prev, newItem];
        }
      ...
      if (!next.some(r => r.item === null)) {
        next.push({ _id: generateId(), item: null, query: '', qty: '1', price: '0', discount: '0', total: '0' });
      }
      return next;
    });
  };
  ...
  const updateRow = (rowId: string, field: 'qty' | 'discount' | 'query', val: string) => {
    setRows(prev =>
      prev.map(r => {
        if (r._id !== rowId) return r;
        const updated = { ...r, [field]: val };
        
        // Handle direct query typing for suggestions
        if (field === 'query') {
          setActiveRowId(rowId);
          if (val.trim()) {
            const filtered = paletteAll.filter(item => 
              item.item_name.toLowerCase().includes(val.toLowerCase())
            ).slice(0, 5);
            setInlineSuggestions(filtered);
          } else {
  ...
  const selectItemFromInline = (rowId: string, item: InventoryItem) => {
    setRows(prev => {
        const updated = prev.map(r => {
            ...
            return {
                ...
                query: item.item_name,
                price: item.price.toString(),
                total: Math.max(0, Math.round(parseNum(r.qty) * item.price - parseNum(r.discount))).toString(),
            };
        });
        if (!updated.some(r => r.item === null)) {
            updated.push({ _id: generateId(), item: null, query: '', qty: '1', price: '0', discount: '0', total: '0' });
        }
        return updated;
    });
    setInlineSuggestions([]);
  ...
  const removeRow = (rowId: string) => {
    setRows(prev => {
        ...
        if (next.length === 0) return [{ _id: generateId(), item: null, query: '', qty: '1', price: '0', discount: '0', total: '0' }];
        return next;
    });
  };
  ...
  const resetPOS = () => {
    setRows([{ _id: generateId(), item: null, query: '', qty: '1', price: '0', discount: '0', total: '0' }]);
    setDiscountRows([]);
    setCustomerName('Umum');
    setCustomerPhone('');
    setCustomerAddress('');
  ...
  const handleEditSale = async (sale: Sale) => {
    ...
    if (sale.customer_id) {
        const { data: cust } = await supabase.from('customers').select('phone, address').eq('id', sale.customer_id).single();
        if (cust) {
  ...
  const executePrint = async (type: DocType, customItems?: SaleItem[]) => {
    if (!lastSale || !settings) return;
    
    if (type === 'DO' && !customItems) {
        const initialSelections: Record<number, number> = {};
        lastSaleItems.forEach((it, idx) => {
            initialSelections[idx] = it.quantity;
        });
        setDoSelectedItems(initialSelections);
        setDoModal(true);
        return;
    }
    ...
    const saleToPrint = {
        ...lastSale,
        customer_phone: customerPhone || lastSale.customer_phone,
        customer_address: customerAddress || lastSale.customer_address
    };
    ...
    const result = await printDocument({
      ...
      settings,
      sale: saleToPrint,
      items: customItems || lastSaleItems,
      config,
    });
  ...
  const renderPreviewContent = () => {
    ...
    const saleToPrint = { ...lastSale, customer_phone: customerPhone, customer_address: customerAddress };
    
    if (Platform.OS === 'web') {
      const html = generatePrintHtml(previewType, settings, saleToPrint, lastSaleItems);
    ...
    <View style={styles.card}>
      ...
      {rows.length === 0 ? (
        <View style={styles.cartEmpty}>
          ...
          <Text style={styles.cartEmptyText}>Belum ada barang.</Text>
        </View>
      ) : isDesktop ? (
        <>
          <View style={styles.tableHead}>
            <Text style={[styles.th, { flex: 2.8 }]}>BARANG</Text>
          ...
          {rows.map(row => (
            <View key={row._id} style={[styles.tableRow, { zIndex: activeRowId === row._id ? 100 : 1 }]}>
              <View style={{ flex: 2.8, position: 'relative' }}>
                <View style={styles.inlineSearchWrapper}>
                    <TextInput 
                        style={styles.inlineSearchInput}
                        placeholder="Tulis nama barang..."
                        value={row.query}
                        onChangeText={(v) => updateRow(row._id, 'query', v)}
                        onFocus={() => {
                            setActiveRowId(row._id);
                            if(row.query.trim()) {
                                const filtered = paletteAll.filter(item => 
                                    item.item_name.toLowerCase().includes(row.query.toLowerCase())
                                ).slice(0, 5);
                                setInlineSuggestions(filtered);
                            }
                        }}
                    />
                    <TouchableOpacity 
                        style={styles.inlinePaletteBtn} 
                        onPress={() => {
                            setActiveRowId(row._id);
                            openPalette();
                        }}
                    >
                        <Ionicons name="apps-outline" size={18} color="#64748B" />
                    </TouchableOpacity>
                </View>

                {activeRowId === row._id && inlineSuggestions.length > 0 && (
                    <View style={styles.inlineSuggestionsBox}>
                        {inlineSuggestions.map(item => (
                            <TouchableOpacity 
                                key={item.id} 
                                style={styles.inlineSuggestItem}
                                onPress={() => selectItemFromInline(row._id, item)}
                            >
                                <Text style={styles.inlineSuggestName}>{item.item_name}</Text>
                                <Text style={styles.inlineSuggestSub}>{formatRupiah(item.price)} • Stok: {item.quantity}</Text>
                            </TouchableOpacity>
                        ))}
                    </View>
                )}
              ...
              <Text style={[styles.mono, styles.cellPrice, { flex: 1.2, textAlign: 'right' }]}>{formatRupiah(parseNum(row.price))}</Text>
              <View
                style={{
                  flex: 1.8,
                  flexDirection: 'row',
              ...
              >
                {discountRows.includes(row._id) || parseNum(row.discount) > 0 ? (
                  <TextInput
                    style={[styles.mono, styles.cellInput, { width: 60, marginBottom: 0, textAlign: 'center', marginRight: 8 }]}
                    keyboardType="numeric"
                    value={row.discount}
                  ...
                  <TouchableOpacity
                    style={[styles.discTagBtn, { marginRight: 8 }]}
        ...
        rows.map(row => (
          <View key={row._id} style={[styles.mItemCard, { zIndex: activeRowId === row._id ? 100 : 1 }]}>
            <View style={styles.mItemHead}>
              <View style={[styles.inlineSearchWrapper, { flex: 1 }]}>
                <TextInput 
                    style={[styles.inlineSearchInput, { fontSize: 14 }]}
                    placeholder="Tulis nama barang..."
                    value={row.query}
                    onChangeText={(v) => updateRow(row._id, 'query', v)}
              ...
              <TouchableOpacity onPress={() => removeRow(row._id)} style={{ marginLeft: 10 }}>
                <Ionicons name="trash-outline" size={22} color="#DC2626" />
              </TouchableOpacity>
            </View>

            {activeRowId === row._id && inlineSuggestions.length > 0 && (
                <View style={[styles.inlineSuggestionsBox, { position: 'relative', top: 0, marginBottom: 10 }]}>
                    {inlineSuggestions.map(item => (
                        <TouchableOpacity 
                            key={item.id} 
                            style={styles.inlineSuggestItem}
      ...
      <TouchableOpacity 
        style={styles.addBtn} 
        onPress={() => setRows(prev => [...prev, { _id: generateId(), item: null, query: '', qty: '1', price: '0', discount: '0', total: '0' }])}
      >
  ...
  const renderCheckout = () => (
    <View style={styles.receiptCard as ViewStyle}>
      ...
      <View style={styles.mb15}>
        ...
        />
        {selectedCustomerId ? <Text style={styles.custLinkedHint}>Terhubung ke database pelanggan</Text> : null}
      </View>
  ...
  return (
    <View style={styles.container}>
        ...
        {activeTab === 'input' ? (
          ...
          <View style={styles.historyContainer}>
            ...
            {loading ? (
              ...
              groupedSales.map(([date, items]) => {
                const dayTotal = items.reduce((a, s) => a + (s.total_amount || 0), 0);
                const isExpanded = expandedDates[date];
                return (
                  <View key={date} style={{ marginBottom: 15 }}>
                    <TouchableOpacity 
                      style={styles.histDateHeader} 
                      onPress={() => toggleDateGroup(date)}
                    >
                      <View style={{ flex: 1 }}>
                        <Text style={styles.histDateLabel}>{date}</Text>
                        <Text style={styles.histDateSub}>
                          {items.length} nota • {formatRupiah(dayTotal)}
                        </Text>
                      </View>
      ...
      {/* Fallback Item Picker Palette */}
      <CommandPalette<InventoryItem>
        visible={paletteOpen}
        onClose={() => { setActiveRowId(null); setPaletteOpen(false); }}
        items={paletteAll}
        ...
        onSelect={(item) => {
          if (activeRowId) {
            selectItemFromInline(activeRowId, item);
          } else {
            addItemToCart(item);
          }
        }}
        ...
        footer={
          <>
            <Text style={{ fontSize: 12, color: '#64748B', fontWeight: '600' }}>{rows.filter(r => r.item).length} barang di keranjang</Text>
            <TouchableOpacity
      ...
      {/* Delivery Order Item Selector Modal */}
      <Modal visible={doModal} transparent animationType="slide">
        <View style={styles.overlay}>
            <View style={[styles.modalCard, { width: '95%', maxWidth: 500, maxHeight: '80%' }]}>
                <Text style={[styles.modalTitle, { marginBottom: 5 }]}>Pilih Barang Dikirim</Text>
                <Text style={{ fontSize: 12, color: '#64748B', marginBottom: 20 }}>Tentukan barang dan jumlah yang dikirim saat ini</Text>
                
                <ScrollView style={{ width: '100%' }}>
                    {lastSaleItems.map((it, idx) => (
                        <View key={idx} style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 15, padding: 10, backgroundColor: '#F8FAFC', borderRadius: 10 }}>
                            <TouchableOpacity 
                                style={{ flex: 1 }}
                                onPress={() => {
                                    setDoSelectedItems(prev => {
                                        const copy = { ...prev };
                                        if (copy[idx] !== undefined) delete copy[idx];
                                        else copy[idx] = it.quantity;
                                        return copy;
                                    });
                                }}
                            >
                                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
                                    <Ionicons name={doSelectedItems[idx] !== undefined ? "checkbox" : "square-outline"} size={24} color="#16A34A" />
                                    <View>
                                        <Text style={{ fontWeight: '700', color: '#0F172A' }}>{it.item_name}</Text>
                                        <Text style={{ fontSize: 11, color: '#64748B' }}>Dibeli: {it.quantity}</Text>
                                    </View>
                                </View>
                            </TouchableOpacity>
                            
                            {doSelectedItems[idx] !== undefined && (
                                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5 }}>
                                    <Text style={{ fontSize: 11, fontWeight: '700' }}>Kirim:</Text>
                                    <TextInput 
                                        style={{ width: 50, backgroundColor: '#FFF', borderWidth: 1, borderColor: '#E2E8F0', borderRadius: 5, padding: 5, textAlign: 'center', fontWeight: 'bold' }}
                                        keyboardType="numeric"
                                        value={String(doSelectedItems[idx])}
                                        onChangeText={(v) => {
                                            const n = parseNum(v);
                                            setDoSelectedItems(prev => ({ ...prev, [idx]: Math.min(it.quantity, n) }));
                                        }}
                                    />
                                </View>
                            )}
                ...
                <View style={{ flexDirection: 'row', gap: 10, marginTop: 20, width: '100%' }}>
                    <TouchableOpacity 
                        ...
                        onPress={() => setDoModal(false)}
                    >
                        <Text style={{ fontWeight: 'bold', color: '#475569' }}>BATAL</Text>
                    </TouchableOpacity>
                    <TouchableOpacity 
                        style={{ flex: 2, padding: 15, borderRadius: 12, backgroundColor: '#16A34A', alignItems: 'center' }}
                        onPress={() => {
                            const selected = Object.entries(doSelectedItems).map(([idx, qty]) => ({
                                ...lastSaleItems[Number(idx)],
                                quantity: qty
                            ...
                            setDoModal(false);
                            executePrint('DO', selected);
                        }}
                    >
                        <Text style={{ fontWeight: 'bold', color: '#FFF' }}>CETAK SURAT JALAN</Text>
                    </TouchableOpacity>
                </View>
            </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  inlineSuggestName: { fontWeight: '700', color: '#0F172A', fontSize: 13 },
  inlineSuggestSub: { fontSize: 11, color: '#64748B', marginTop: 2 },
  ...
  addBtnText: { fontWeight: '800', fontSize: 12, color: '#0F172A' },
  ...
  histDateLabel: { fontSize: 12, fontWeight: '900', color: '#0F172A' },
  histDateSub: { fontSize: 10, fontWeight: '700', color: '#94A3B8' },
  historyCard: {
    backgroundColor: '#FFF',
    borderRadius: 10,
    marginBottom: 8,
  ...
  histStrip: { position: 'absolute', left: 0, top: 0, bottom: 0, width: 3 },
  histHead: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
  ...
  histDetailRow: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 4 },
  histDetailName: { flex: 1, fontSize: 12, color: '#334155', marginRight: 8 },
  histDetailVal: { fontSize: 12, fontWeight: '700', color: '#0F172A' },
  histDetailActions: { flexDirection: 'row', gap: 6, marginTop: 10 },
  histActBtn: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 4,
  ...
  histActText: { fontSize: 11, fontWeight: '700', color: '#0F172A' },
  hRemaining: { fontSize: 10, color: '#B45309', fontWeight: '700', marginTop: 1 },
  histSearch: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 10,
  ...
  histSearchInput: { flex: 1, fontSize: 14, color: '#0F172A' },
  ...
  hName: { fontWeight: '800', fontSize: 13, color: '#0F172A' },
  hDate: { fontSize: 9, color: '#94A3B8', marginTop: 1 },
  hPrice: { fontSize: 13, fontWeight: '700', color: '#0F172A' },
  hMeta: { flexDirection: 'row', gap: 4, marginTop: 6, alignItems: 'center' },
  badge: { fontSize: 8, fontWeight: '900', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4 },
